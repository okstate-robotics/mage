...
class UAV:
	...
	def UAV_init(self,Sensor):
		...
		self.Ps = array([[0],[0],[500],[45*pi/180]])
		self.Pe = array([[1500],[1500],[550],[45*pi/180]])
	...
	def UAV_Run(self,Sensor):	
		...
		# Position data
		p = array([ [Sensor.Pos_n],[Sensor.Pos_e],[Sensor.Pos_d] ])
...
def GC_main(argv):
	...
	uav_sub=rospy.Subscriber(Sub_Topic_name, sensor_data, uav.UAV_init,queue_size=10) # Get data from Sensor_data topic and initialize the definitions
	sleep(1) 
	uav_sub.unregister()
	sleep(4)
	Sub_Topic_name = 'Sensor_data_'+UAV_callsign  ### MAKE SURE TO PASS UAV_callsign AS AN ARG TO THE FNC
	uav_sub=rospy.Subscriber(Sub_Topic_name, sensor_data, uav.UAV_Run,queue_size=10) # Get data from Sensor_data topic and execute the guidance & control loop
