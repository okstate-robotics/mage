...
class Guidance:
	...
	def set_wp(self,Ps,Pe): # oldname was check_wp(self,Ps,Pe)
		
		if any(Ps != self.Ps_existing) or any(Pe != self.Pe_existing): # check if Ps and Pe are different from existing ones
...
class Controller:
	...
	def control(self,sensor_in,h_c,X_c,V_c,Heading_c=45*pi/180,state=1):
		x_pos = sensor_in[0]
		y_pos = sensor_in[1]
		z_pos = sensor_in[2]		
		x_vel = sensor_in[3]
